		...
		<div class="main">
			...
			<div class="search">
				<form id="addContentForm">
					<div class="row">
						<div class="col-md-3 required">seqNO</div>
						<div class="col-md-6"><input required="required" maxLength="8" digits="true" type="text" name="seqNo"></div>
					</div>
					<div class="row">
				...
				<div class="tab" id="tab">
					...
					<div class="tab_box">
						<c:forEach items="${languages}" var="language" varStatus="status">
							<c:choose>
								<c:when test="${status.index == 0 }">
									<div class="box" style="height:400px"  id="divlan${status.index}">
								...
								<c:otherwise>
									<div class="hide box" style="height:400px" id="divlan${status.index}">
										<form class="language" id="language${status.index}">
											<input type="hidden" name="lang"
												value="${language.localeId }" />
												<pageTag:datatype-edit datatypeId="${datatypeId}" lang="${language.localeId }"/>
...
<script type="text/javascript">
	...
	jQuery.validator.addMethod("isFieldValueUnique", function(value, element) { 
	    ...
	    if(element.value!=''){
		    $.ajax({
				...
				data:"fieldname="+element.name+"&fieldvalue="+element.value+"&lang="+element.getAttribute('isFieldValueUnique'),
	            async:false,
	            type:'POST',
	            //dataType:"json",
	...
	$(function() {
		...
		<c:forEach items="${languages}" var="language" >
			//UeditorFileLoad.init("textareaedit${language.localeId }",330,80);
			/*$("#textareaedit${language.localeId }").each(function(){
		...
		$("[id=actionSearch]").each(function(){
			if($(this).attr('name')=='SeqNo'){
				...
				$(this).attr("digits","true");
				if('${d.datatypeName}'=='Hot Spots'){
					$(this).attr("required","true");
					var div = $(this).parent('div').parent('div').find('div').eq('0');
					div.attr("class",div.attr('class')+" required");
				}
			}
			
			
			if($(this).attr('name')=='Image URL'){
				$(this).attr("maxlength","250");
			}
			if($(this).attr('name')=='Image Alt'){
				$(this).attr("maxlength","250");
			}
			
			if($(this).attr('name')=='Target URL' ||$(this).attr('name')=='File URL(for external)' ){
				$(this).attr("maxlength","250");
			}
			
			if($(this).attr('name')=='Image Overlay Text'){
				$(this).attr("maxlength","500");
			}
			
			if($(this).attr('name')=='Description'||$(this).attr('name')=='Content'){
		...
		$('#language0 [id=actionSearch]').each(function(){
			...
			if('${d.datatypeName}'!='Hot Spots'&&$(this).attr('name')=='Target URL'){
				$(this).attr("required","true");
				var div = $(this).parent('div').parent('div').find('div').eq('0');
				div.attr("class",div.attr('class')+" required");
			}
			
			if($(this).attr('name')=='File URL(for external)'){
				$(this).attr("required","true");
				var div = $(this).parent('div').parent('div').find('div').eq('0');
				div.attr("class",div.attr('class')+" required");
			}
