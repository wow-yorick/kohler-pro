...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('.createPage').on('click', function(){
		    $.layer({
		        ...
		        iframe: {src: '<%=request.getContextPath()%>/section/selectPageType.action?sectionId=${sectionId}'}
		    });
		});
	...
	function initEditSectionPage(sectionId,sectionType){
		if(sectionType == "Section"){
			alertFirstIframe("Section","edit.action?sectionId="+sectionId+"&isEdit=1",'700px','525px');
		}else{
			alertFirstIframe("Page","editPage.action?pageId="+sectionId,'880px','650px');
		}
	}
	...
	function initViewSectionPage(sectionId,sectionType,viewType){
		...
		if(sectionType == "Section"){
			...
			alertFirstIframe("Section","editPage.action?pageId="+sectionId+"&isEdit=0",'880px','650px');
		}
	}
	...
	function deleteSectionOrPage(){
		...
		$.post("deleteSectionOrPage.action", "data="+jsonData,function(data) {
