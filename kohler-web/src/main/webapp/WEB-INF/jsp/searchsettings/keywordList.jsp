...
<script type="text/javascript" src="<%=request.getContextPath()%>/js/jquery.ztree.all-3.5.min.js"></script>
    ...
    function updateSpelling(spellingId){
    	alertFirstIframe("update","edit.action?searchKeywordRedirectionId="+spellingId+"&isEdit=0",'840px','600px');
    }
    function searchSpelling(spellingId){
    	alertFirstIframe("update","edit.action?searchKeywordRedirectionId="+spellingId+"&isEdit=1",'840px','600px');
    }
    function deleteSeplling(){
    	var jsonData = "";
		var chooses = $("input[name='searchKeywordRedirectionId']:checked");
		alert(chooses);
		if(chooses.length == 0){
	...
	<div class="wrap newproducts">
		...
		<div class="content">
			...
			<div class="content_main">
				<!-- 操作及筛选区域condition开始-->	
				<div class="condition">
					<h1>keyword redirectionUrl</h1>
					<form id="searchForm" action="searchKeywordRedirectionList.action" method="post">	
						<div class="query row">
							<div class="col-md-10">
								<div class="col-md-1"><label>Name</label></div>
								<div class="col-md-2"><input type="text" name="keyword" value="${v}"></div>						
							</div>
					...
					<table cellspacing="0">
						<thead>
							<tr>
								...
								<th  class="no_rightborder" style="width:60%">Redirection URL</th>
							</tr>
						</thead>
						<tbody> 
								...
								<tr>
									<td>
										<div class="checkbox">
											...
											<input type="checkbox" name="searchKeywordRedirectionId" value="${l.searchKeywordRedirectionId}">
										</div>
										<a href="#" class="ope_icon edit" onclick="updateSpelling('${l.searchKeywordRedirectionId}');"></a>
										<a href="#" class="ope_icon detail" onclick="searchSpelling('${l.searchKeywordRedirectionId}');"></a>
									</td>
	                                <td class="fl_left">${l.keyword}</td>
									<td class="no_rightborder fl_left">${l.redirectionUrl}</td>
								</tr>	
							</c:forEach>		
						</tbody>
					</table>
