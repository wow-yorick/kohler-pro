...
<div class="container queryConditions product_layer">
    ...
    <form id="detailForm" action="">
    	<input type="hidden" id="suiteProductId" value="${suiteProduct.suiteProductId}" />
    	...
    	<div class="search">
            ...
            <div class="row">
                ...
                <div class="col-md-4 "><input type="text" disabled="disabled" id="skuCodeName" name="skuCode" value="${suiteProduct.skuCode}" />
                <input type="hidden" id="skuMetadataId"  name="skuMetadataId" value="${suiteProduct.skuMetadataId}"  /></div> 
                <div class="col-md-2">
                    <a href="javascript:void(0);" class="choose tn_c" onclick="openSkuPicker('skuMetadataId',1,'skuCodeName')">Choose</a>
                </div>
            </div>
...
function chooseSaveAction() {
	...
	//var suiteProId = $('#suiteProductId').val();
...
/* function savaSuiteProductDB() {
	...
	$.post("<%=request.getContextPath()%>/suite/unlimited/editSave.action", detailForm.serialize(), 
			function(data) {
				if(!data.success) {
					alert(data.msg);
...
console.log(parentOldInfoObj.length);
	parentOldInfoObj.find('[name=skuMetadataId]').val(skuMetadataId);
	parentOldInfoObj.find('[name=seqNo]').val(seqNo);
	parentOldInfoObj.find('[data-skuName]').val(skuCode);
	parentOldInfoObj.parent().parent('tr').find('.last').text(skuCode);
	parent.skuAction.evenHighlight();
	parent.layer.close(index);
}
