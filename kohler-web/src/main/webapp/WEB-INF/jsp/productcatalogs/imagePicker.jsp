...
$(function(){
	var eleIds = parent.document.getElementById('${elementId}').value;
	if(eleIds!=null&&eleIds!=""){
		$.post("searchImgContent.action",{'fileAssetIds':eleIds},function(data){
			var html = "";
			$.each(data.asset,function(name,value){
			   html += '<tr>'
			        +'<td class="tn_c"><span class="minus" onclick="deleteSkuSelt(this)"></span><input type="hidden" name="fileAssetId" class="selectSkuIds" value="'+value.fileAssetId+'"></td>'
			        +'<td>'+value.fileAssetName+'<input type="hidden" class="selectSkuIds" name="fileAssetName" value="'+value.fileAssetName+'"></td>'
			    	+'</tr>';
			});
			 html += "</tr>";
            	...
            	<td colspan="2">
                    <table class="small" border="0" cellspacing="0" cellpadding="0">
                        ...
                        <c:forEach items="${fileLsit}" var="li">
	                        <tr>
	                            <td class="tn_c"><span class="plus" onclick="add('${li.fileAssetId}','${li.fileAssetName}',this);"><input type="hidden" idstr="${li.fileAssetId}"></input></span></td>
